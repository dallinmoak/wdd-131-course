...
const recipeSectionTemplate = (data) => {
  return `
  <section>
...
const renderRecipes = (recipes) => {
  const recipeContainerEl = document.querySelector("main");
  recipeContainerEl.innerHTML = "";
  recipes.forEach((recipe) => {
    recipeContainerEl.innerHTML += recipeSectionTemplate(recipe);
  });
};
...
const handleSearch = (e) => {
  ...
  const filteredRecipes = recipes.filter((r)=>{
    ...
    const descriptionMatch = r.description.toLowerCase().includes(searchQuery);
    let tagsMatch = false;
    r.tags.forEach((t) => tagsMatch = t.toLowerCase().includes(searchQuery))
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  const initialRecipes = [recipes[randomIndex]]
